...
class LLMService:
    ...
    async def analyze_text(
        ...
        """
        Analyze text content (like comments) to determine if it should generate a ticket.
        Uses similar logic to email analysis but adapted for general text.
        """
        try:
            # Check if LLM is available
            if self.llm_provider is None:
                return await self._keyword_based_analysis("", text)
            
            # Build the prompt for text analysis
            prompt = self._build_text_analysis_prompt(text, context)
...
Determine if this is SAP-related, classify the module, assess priority, and suggest a ticket title."""
    ...
    def _build_text_analysis_prompt(self, text: str, context: str = "") -> str:
        """Build the prompt for text analysis (comments, etc.)"""
        max_text_length = 3000
        truncated_text = text[:max_text_length] + "..." if len(text) > max_text_length else text
        
        context_info = f"\nCONTEXT: {context}" if context else ""
        
        return f"""Analyze this text for potential SAP support ticket creation:

TEXT:
{truncated_text}{context_info}

Determine if this text contains information that should generate a new SAP support ticket.
Consider if it mentions new issues, problems, or requirements that aren't already being addressed.

Respond with the same JSON format as email analysis."""
