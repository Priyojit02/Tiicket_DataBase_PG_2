...
class TicketService:
    ...
    async def create_ticket(
        ...
        try:
            ...
            # Analyze the manually created ticket with LLM for validation/enhancement
            try:
                analysis_result = await self.llm_service.analyze_text(
                    text=f"{ticket_data.title}\n\n{ticket_data.description or ''}",
                    context=f"Manually created ticket by {current_user.name}"
                )
                
                # Update ticket with LLM analysis results if they improve the data
                if analysis_result.is_sap_related and analysis_result.confidence > 0.5:
                    update_data = {}
                    
                    # Use LLM-suggested priority if it's different and more appropriate
                    if analysis_result.suggested_priority and analysis_result.suggested_priority != ticket_data.priority:
                        update_data["priority"] = analysis_result.suggested_priority
                    
                    # Use LLM-detected category if it's different and more specific
                    if analysis_result.detected_category and analysis_result.detected_category != ticket_data.category:
                        update_data["category"] = analysis_result.detected_category
                    
                    # Update ticket if we have improvements
                    if update_data:
                        await self.ticket_repo.update(ticket.id, update_data)
                        ticket = await self.ticket_repo.get_with_details(ticket.id)
                        print(f"LLM enhanced manual ticket {ticket_id}: {update_data}")
                    
                    print(f"LLM validated manual ticket {ticket_id} as SAP-related (confidence: {analysis_result.confidence})")
                else:
                    print(f"LLM analysis of manual ticket {ticket_id}: Not SAP-related or low confidence ({analysis_result.confidence})")
                    
            except Exception as e:
                print(f"LLM analysis failed for manual ticket {ticket_id}: {e}")
...
"""
    ...
    async def add_comment(
        ...
        # Analyze comment content for potential ticket generation
        try:
            analysis_result = await self.llm_service.analyze_text(
                text=comment_data.content,
                context=f"Comment on ticket {ticket.ticket_id}: {ticket.title}"
            )
            
            # If comment is SAP-related and has sufficient confidence, create a new ticket
            if analysis_result.is_sap_related and analysis_result.confidence > 0.3:  # Lowered threshold
                print(f"Comment analysis: SAP-related (confidence: {analysis_result.confidence}) - Creating ticket")
                print(f"Analysis details: category={analysis_result.detected_category}, priority={analysis_result.suggested_priority}, title='{analysis_result.suggested_title}'")
                
                # Create a new ticket based on the comment
                new_ticket_data = {
                    "title": analysis_result.suggested_title or f"Follow-up: {comment_data.content[:50]}...",
                    "description": f"Generated from comment on ticket {ticket.ticket_id}:\n\n{comment_data.content}",
                    "status": "Open",
                    "priority": analysis_result.suggested_priority.value,
                    "category": analysis_result.detected_category.value if analysis_result.detected_category else "OTHER",
                    "created_by": current_user.id,
                    "assigned_to": ticket.assigned_to_id,  # Assign to same person as parent ticket
                    "completion_by": datetime.now() + timedelta(days=7)  # Default 7 days
                }
                
                new_ticket = await self.create_ticket(new_ticket_data, current_user)
                if new_ticket:
                    print(f"New ticket {new_ticket.ticket_id} created from comment analysis")
                    
                    # Add a log entry to the original ticket
                    await self._create_log(
                        ticket_id=ticket_id,
                        user_id=current_user.id,
                        log_type=LogType.COMMENT,
                        action=f"New ticket {new_ticket.ticket_id} generated from comment"
                    )
                    
                    # Update frontend tickets file
                    await self.update_frontend_tickets_file()
            else:
                print(f"Comment analysis: Not creating ticket - SAP-related: {analysis_result.is_sap_related}, confidence: {analysis_result.confidence}")
                if analysis_result.detected_category:
                    print(f"Detected category: {analysis_result.detected_category}")
                if analysis_result.suggested_title:
                    print(f"Suggested title: {analysis_result.suggested_title}")
                
        except Exception as e:
            print(f"Error analyzing comment for ticket generation: {e}")
            # Continue without creating ticket - don't fail the comment
