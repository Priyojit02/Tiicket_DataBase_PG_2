...
async function loadLLMTickets(): Promise<Ticket[]> {
    try {
        console.log('🤖 loadLLMTickets: Starting dynamic import of tickets2.ts');
        // Use dynamic import for Next.js compatibility
        const tickets2Module = await import('@/data/tickets2');
        console.log('🤖 loadLLMTickets: Import successful, calling getTicketsData()');
        const llmTickets = tickets2Module.getTicketsData(); // Use function to get fresh data
        console.log(`🤖 loadLLMTickets: getTicketsData() returned ${llmTickets.length} tickets`);
        
        return llmTickets;
    } catch (error) {
        console.error('❌ loadLLMTickets: Failed to load tickets2.ts:', error);
        // tickets2.ts not available, return empty array
        console.log('ℹ️ loadLLMTickets: tickets2.ts not found, using empty array');
        return [];
    }
}
...
async function loadTickets(): Promise<Ticket[]> {
    const llmTickets = await loadLLMTickets(); // Now includes user-created tickets
...
export async function getAllTickets(): Promise<ApiResponse<Ticket[]>> {
    if (USE_MOCK_DATA) {
        // Use mock data if available, otherwise fall back to initialTickets
        if (mockTickets.length > 0) {
            return { success: true, data: mockTickets };
        } else {
            console.log('🔄 USE_MOCK_DATA=true but mockTickets empty, using initialTickets');
            return { success: true, data: initialTickets };
        }
    }
    ...
    try {
        ...
        // Fallback to mock data on error - use initialTickets as immediate fallback
        if (mockTickets.length > 0) {
            return { success: true, data: mockTickets };
        } else {
            console.log('🔄 Using initialTickets as fallback');
            return { success: true, data: initialTickets };
        }
    }
}
...
export async function getTicketById(id: number | string): Promise<ApiResponse<Ticket>> {
    if (USE_MOCK_DATA) {
        const ticket = mockTickets.find((t: Ticket) => t.id === Number(id)) || 
                      initialTickets.find((t: Ticket) => t.id === Number(id));
        return ticket 
...
export async function createTicket(payload: CreateTicketPayload): Promise<ApiResponse<Ticket>> {
    if (!USE_MOCK_DATA) {
        try {
            ...
            // Use proper data transformation for backend
            const backendData = toBackendCreateTicket({
                title: payload.title,
                description: payload.description || '',
                priority: payload.priority || 'Medium',
                module: payload.module || 'OTHER', // Map module to category
            });
            
            const response = await ticketsApi.createTicket(backendData);
            console.log('✅ Backend API returned ticket data');
            // Transform backend response to frontend format
            const transformedTicket = transformTicket(response as any);
            console.log('✅ Ticket transformed and ready for frontend');
    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            ...
            // Persist user-created ticket to localStorage
            if (typeof window !== 'undefined') {
                try {
                    ...
                    console.log('💾 createTicket: Saved to localStorage:', newTicket.title);
                    console.log('💾 createTicket: Total tickets now:', updatedTickets.length);
                    console.log('💾 createTicket: localStorage contents:', localStorage.getItem('userCreatedTickets'));
                } catch (error) {
                    console.error('❌ createTicket: Failed to save to localStorage:', error);
                }
            } else {
                console.log('⚠️ createTicket: localStorage not available');
            }
...
export async function addComment(
    ...
    if (!USE_MOCK_DATA) {
        try {
            await ticketsApi.addComment(id, commentText, false);
            // After adding comment, fetch the updated ticket
            return await getTicketById(id.toString());
        } catch (error: unknown) {
...
/**
 * Clear all user-created tickets from localStorage
 */
export function clearUserCreatedTickets(): void {
    if (typeof window !== 'undefined') {
        try {
            localStorage.removeItem('userCreatedTickets');
            // Reload mockTickets to reflect the change
            initializeTickets().catch(console.error);
            console.log('🗑️ User-created tickets cleared from localStorage');
        } catch (error) {
            console.error('Failed to clear user tickets from localStorage:', error);
        }
    }
}
